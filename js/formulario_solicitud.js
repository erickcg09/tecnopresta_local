  ...
  if (userDataCanasta && userDataCanasta.length>0) {
    ...
    jsonData.forEach(obj => {
          ...
          */
          ...
          botonQuitar.innerHTML = '<span class="font-weight-bold text-danger" aria-hidden="true">&times;</span>';
...
function cargaDatosPantallaArticulo(userDataArticulo = []) {
  ...
  if (userDataArticulo && userDataArticulo.length>0) {
    ...
    jsonDataArticulo.forEach(obj => {
          ...
          let divcontenedorCantidadQuitar = document.createElement('div');
          contenedorCantidadQuitar.appendChild(divcontenedorCantidadQuitar);
          ...
          botonQuitar.onclick = new Function("botonQuitarArticulo(" + JSON.stringify(obj) + ");");
          botonQuitar.innerHTML = '<span class="font-weight-bold text-danger" aria-hidden="true">&times;</span> ';
...
function cargaDatosPantallaBuscar(rs) {
  ...
  rs.forEach(obj => {
    ...
    //Fila Col Imagen
    let filaImagen = document.createElement('div');
    filaImagen.className = "row";

    let colImagen = document.createElement('div');
    colImagen.className = "col-sm";
    ...
    filaImagen.appendChild(colImagen);
    contenedorTituloImagen.appendChild(filaImagen);
    ...
    let selectCantidadsolicitud = document.createElement('select');
    selectCantidadsolicitud.className="col-4 cssCombo";
    ...
    .then(function(response) {
      ...
      if(response.ok) {
        ...
        response.json().then(function(data) {
          ...
          colCantidadequipos.innerHTML = '<strong>Cantidad equipos: </strong>' + 
    ...
    filaCantidadsolicitud.className = "form-group row";

    let labelCantidadsolicitud = document.createElement('label');
    labelCantidadsolicitud.className="col-4 col-form-label";
    labelCantidadsolicitud.for="cboCantidad";
    let createATextlabelCantidadsolicitud = document.createTextNode("Solicitar:");
    labelCantidadsolicitud.appendChild(createATextlabelCantidadsolicitud);
      
    filaCantidadsolicitud.appendChild(labelCantidadsolicitud);
    ...
    */
     ...
     //botonSolicitud.onclick = function() { agregar(obj, selectCantidadsolicitud.value); };
     botonSolicitud.onclick = function() { agregar(obj, 1); };    
...
function cargaDatosPantallaBuscarArticulo(rs) {
  ...
  rs.forEach(obj => {
    ...
    let h4 = document.createElement('h4');
    h4.className = "card-title";
    let createATextNombre = document.createTextNode(obj.clase + " " + obj.marca + " " + obj.modelo + " " + obj.placa + " " + obj.numero_activo);
    h4.appendChild(createATextNombre);
