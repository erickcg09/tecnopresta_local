...
let global_visitas_procedimiento_id = 0;
                    ...
                    if(response.ok){        
                        response.json().then(function(data){
                            ...
                            if (Object.keys(data).length>0) {                                                                                        
                                data.forEach(obj => {
                                    console.log(data);
                                    document.getElementById("visitas_sitio_hoja_trabajo_resultado").value=obj.visitas_sitio_hoja_trabajo_resultado;
                                    document.getElementById("visitas_sitio_hoja_trabajo_indicaciones").value=obj.visitas_sitio_hoja_trabajo_indicaciones;                                 
...
function carga_pantalla_hoja_trabajo(id_visita) {
    // Mostrar spinner si lo usas
    let spinner = document.getElementById("spinner");
    if (spinner) spinner.className = "spinner-border";
    ...
    .then(data => {
        if (Array.isArray(data) && data.length > 0) {
            data.forEach(obj => {
                ...
                document.getElementById("visitas_sitio_hoja_trabajo_indicaciones").value = obj.visitas_sitio_hoja_trabajo_indicaciones || "";
                document.getElementById("segundavisita").checked = !!obj.requiere_segunda_visita;
                document.getElementById("tiempo_atencion_hora").value = obj.horas_total_atencion || 0;
                document.getElementById("tiempo_atencion_minuto").value = obj.minutos_total_atencion || 0;
            });
        } else {
            //console.warn("No se encontraron datos para la visita:", id_visita);
        }
    })
    .catch(error => {
        let contenedorError = document.getElementById("mensaje");
    ...
    .finally(() => {
        if (spinner) spinner.className = "d-none";
    });
}
...
function obtiene_hoja_trabajo_id_para_cargar_procedimiento(id_visita) {
    try {
        fetch('sql/select_visitas_sitio_hoja_trabajo_id_Gestor.php?'
                    + new URLSearchParams({id_visita: id_visita}))
                .then(function(response){                   
                    if(response.ok){        
                        response.json().then(function(data){
                            ...
                            if (visitas_sitio_hoja_trabajo_id>0) {
                                carga_pantalla_procedimiento(visitas_sitio_hoja_trabajo_id);    
                            }
...
function carga_pantalla_procedimiento(visitas_sitio_hoja_trabajo_id) {
    try {
        fetch('sql/select_lista_procedimiento_Gestor.php?'
            + new URLSearchParams({visitas_sitio_hoja_trabajo_id: visitas_sitio_hoja_trabajo_id}))
            .then(function(response){                   
            if(response.ok){        
                response.json().then(function(data){
                    if (Object.keys(data).length>0) {
                        //console.log(data);                                                
                        cargaDatosPantalla_procedimiento(JSON.stringify(data));                                                     
                    }                                                                               
                });                    
            }        
        }).then();                    
  ...
  try {
    ...
    data.forEach(obj => {                   
     ...
     if (id_visita > 0) {
        ...
        obtiene_hoja_trabajo_id_para_cargar_procedimiento(id_visita);
...
function verifica_antes_guardar() {
    ...
    if (arrlista_de_indicadores.length>0) {
        visitas_sitio.lista_de_indicador_procedimiento=arrlista_de_indicadores;
    } else {
        visitas_sitio.lista_de_indicador_procedimiento=[];
    }
    ...
    let visitas_sitio_hoja_trabajo_resultado = document.getElementById('visitas_sitio_hoja_trabajo_resultado').value;
    const visitas_sitio_hoja_trabajo_resultado_limpio = visitas_sitio_hoja_trabajo_resultado.trim();
    if (visitas_sitio_hoja_trabajo_resultado_limpio.length > 0) {
        const regex = /^[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ\s.,;:!?()\-+@#\/="'&%$^*_\[\]{}<>|~`]+$/;
        if (!regex.test(visitas_sitio_hoja_trabajo_resultado_limpio)) {
            let contenedorError = document.getElementById("mensaje");    
            contenedorError.innerHTML='<div class="alert alert-danger">' +
                                            '<strong>Error al guardar! </strong>' +
                                                'El texto de Resultado Final contiene caracteres no permitidos ...' +
                                            '</div>';
            let spinner = document.getElementById("spinner");           
            spinner.className="d-none";                                        
            return false;  
    ...
    let visitas_sitio_hoja_trabajo_indicaciones = document.getElementById('visitas_sitio_hoja_trabajo_indicaciones').value;
    const visitas_sitio_hoja_trabajo_indicaciones_limpio = visitas_sitio_hoja_trabajo_indicaciones.trim();
    if (visitas_sitio_hoja_trabajo_indicaciones_limpio.length > 0) {
        const regex = /^[a-zA-Z0-9áéíóúÁÉÍÓÚñÑ\s.,;:!?()\-+@#\/="'&%$^*_\[\]{}<>|~`]+$/;
        if (!regex.test(visitas_sitio_hoja_trabajo_indicaciones_limpio)) {
            let contenedorError = document.getElementById("mensaje");    
            contenedorError.innerHTML='<div class="alert alert-danger">' +
                                            '<strong>Error al guardar! </strong>' +
                                                'El texto de Indicaciones Adicionales contiene caracteres no permitidos ...' +
                                            '</div>';
            let spinner = document.getElementById("spinner");           
            spinner.className="d-none";                                        
            return false;  
    ...
    visitas_sitio.visitas_sitio_hoja_trabajo_resultado=visitas_sitio_hoja_trabajo_resultado_limpio;
    visitas_sitio.visitas_sitio_hoja_trabajo_indicaciones=visitas_sitio_hoja_trabajo_indicaciones_limpio;
...
function cargaDatosBd_Procedimiento() {
    ...
    muestra_Procedimiento_bd(valor);
...
function cargaDatosBd_Buscar_Articulo(codigoPresupuestario) {
  ...
  function muestra_Procedimiento_bd(valor) {

    fetch('sql/selectProcedimiento_Gestor.php?'
...
function btn_gregarArticulo() {
    ...
    checkboxes.forEach(checkbox => {
        let raw = checkbox.getAttribute('data-obj');
        if (raw) {            
            arrayArticulos.push(JSON.parse(raw));            
        }
    });

    if (Object.keys(arrayArticulos).length>0) {

        arrayArticulos.forEach(obj => {                                                         
            if (agregarArticulo(obj)!=false) {
                cargaDatosPantallaArticulo_desde_btn_Articulos(obj);
                cerrarModal();
...
function agregarArticulo(jsonArray) {    
    ...
    if (buscarRepetidoArticulo(jsonArray.id_activo, jsonArray.id_placa)) {
      ...
      let contenedorError = document.getElementById("mensajeModalBuscar");
      contenedorError.innerHTML='<div class="alert alert-danger">' +
                              '<strong>El artículo Placa: ' 
                              + jsonArray.placa + ' ya lo agregaste a la Hoja de Trabajo !!</strong>' +
                              '</div>';
      return false;
    }
...
function agregarIndicadores(jsonArray) {
    ...
    if (buscarRepetidoIndicador(jsonArray["visitas_procedimiento_id"])) {
...
function cargaDatosPantallaBuscarArticulo(rs) {
    ...
    rs.forEach(obj => {
       ...
       */    
       ...
       formCheck.className = "col-auto text-center form-check";

       let formCheckInput = document.createElement('input');
       formCheckInput.className = "form-check-input fs-2 border-black";
       formCheckInput.type = "checkbox";
       formCheckInput.id = "formCheckInput-" + obj.id_placa;
       formCheckInput.setAttribute("data-obj", JSON.stringify(obj));
    
       let labelInput = document.createElement('label');
       labelInput.className = "form-check-label fs-4 text-primary fw-bold";
       labelInput.htmlFor = formCheckInput.id;
       labelInput.textContent = "Seleccionar";

       formCheck.appendChild(formCheckInput);
       formCheck.appendChild(labelInput);

       filaBotonsolicitud.appendChild(formCheck);
  ...
  function cargaDatosPantallaBuscaIndicadores(rs) {
    ...
    rs.forEach(obj => {
      ...
      let h4 = document.createElement('h4');
      h4.className = "card-title";
      let createATextNombre = document.createTextNode(obj.visitas_procedimiento_descripcion);
      h4.appendChild(createATextNombre);
  ...
  function cargaDatosPantallaArticulo_desde_btn_Articulos(obj) {
  ...
  function cargaDatosPantallaArticulo(data) {
    ...
    if (data && data.length>0) {
        ...
        let spinner = document.getElementById("spinnerProcedimiento");
        spinner.className="d-block";
       ...
       spinner = document.getElementById("spinnerProcedimiento");           
       spinner.className="d-none";
    ...
    if (data && data.length>0) {
        ...
        jsonDataArticulo.forEach(obj => {
            ...
            let plantilla = `<div class=indicadores row border-top border-primary mb-2"
                                id="${obj.visitas_procedimiento_id}" data-id="${obj.visitas_procedimiento_id}">                
                                <div class="col-10 col-md-10 col-lg-10">
                                    ${obj.visitas_procedimiento_descripcion}
                                </div>
                                <div class="col">
                                <button class="btn btn-outline-danger mt-1"
                                        onclick="botonQuitarIndicador(\'${obj.visitas_procedimiento_id}\');">
                                    <i class="bi bi-trash"></i>
                                </button>
                                </div>            
                            </div>`;
  ...
  function cargaDatosPantalla_procedimiento(data) {
  ...
  function botonQuitarIndicador(id) {
    ...
    // Variable Global.
    global_visitas_procedimiento_id=id;
  ...
  function quitarElementoArrayIndicador() {
    ...
    let cards = document.getElementsByClassName("indicadores");

    for (let i = 0; i < cards.length; i++) {

        let dataActivo_id = cards[i].getAttribute('data-id');

        if (dataActivo_id==global_visitas_procedimiento_id) {        
            let elem = document.getElementById(dataActivo_id);
            elem.parentElement.removeChild(elem);            
        }
...
function lista_de_Indicadores() {
    ...
    indicadores.forEach((indicador) => {
        let visitas_indicador_id = indicador?.getAttribute('data-id') || 0;
        arrlista_de_indicadores.push({visitas_indicador_id});
      });
...
function ir_a_hoja_visita() {
  ...
  function descargarPDF() {
    ...
    window.location.assign("sql/mgmtPDF.php?id_visita="+id_visita);
