...
async function inicioSesion(indice) {
    ...
    try {
        ...
        if (result.ok) {
            ...
            sessionStorage.setItem('sesion', json); // Guarda en sessionStorage para uso en frontend

            // Primero crear Auth
            const authResponse = await fetch('sql/formulario_principal.php', {
                credentials: 'include' // Asegura enviar cookies para mantener sesión
            });
            
            const authData = await authResponse.json();

            console.log("AUTH:", authData); // Verificar que la sesión se creó correctamente en PHP

            if (authData.Ok) {
                // Ahora si puede navegar seguro
                console.log("Auth creada correctamente");
                window.location.replace('navegar.php?ruta=formulario_menu_principal.php');
            } else {
                console.error(authData.error);
                alert("Error autenticando sesión en PHP. Por favor intente de nuevo.");
            }
...
function cargaModal() {
    ...
    jsonData.forEach(obj => {
        ...
        linkList.setAttribute('href', "navegar.php?ruta=formulario_menu_principal.php");
