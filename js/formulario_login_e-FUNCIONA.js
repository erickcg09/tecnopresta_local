...
window.loadPage = async function () {
    ...
    console.log("Cuentas detectadas:", currentAccounts);
    console.log("TOTAL CUENTAS:", currentAccounts.length);
...
function handleResponse(resp) {
    if (resp !== null) {
        let username = resp.account?.username || window.myMSALObj.getAllAccounts()[0]?.username;
        login(username);           
    } else {
        loadPage();
    }
...
window.signInEspecial = function () {
    ...
    /*window.myMSALObj.loginPopup(loginRequest).then(handleResponse).catch(error => {
        console.error(error);
        })*/
...
async function inicioSesion(indice) {
    ...
    try {
        ...
        const response = await fetch('sql/sesionCargaSW.php', {
            method: 'POST',
            body: formData,
            credentials: 'include' //para sesiones PHP
        });
        ...
        if (result.ok) {
            ...
            if (foto) {
                console.log("📸 Enviando foto a PHP...");
                await guardarFotoEnPHP(foto);
                //console.log("✅ Foto guardada en sesión PHP");
            }
            // Redirigir
            // window.location.replace('formulario_menu_principal.html');
...
async function guardarFotoEnPHP(foto) {
    ...
    try {
        ...
        console.log(" Guardando foto en PHP...");
