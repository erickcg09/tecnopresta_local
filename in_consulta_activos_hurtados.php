...
if (isset($_POST['id_fondos'])) {
    ...
    // Consulta SQL para contar el total de activos robados por institución filtrado por id_fondos
    $count_sql = "
        SELECT t_instituciones.codigo, t_instituciones.institucion, COUNT(t_placa.codigo) AS total_activos_robados
        FROM t_placa
        INNER JOIN t_instituciones ON t_placa.codigo = t_instituciones.codigo
        WHERE t_placa.id_estado = 5 AND t_placa.id_fondos = ?
    ...
    if ($stmt = $link->prepare($count_sql)) {
        ...
        if ($result->num_rows > 0) {
            ...
            echo '<thead><tr><th>Código</th><th>Institución</th><th>Total de Activos Robados</th></tr></thead>';
            echo '<tbody class="BusquedaRapida">';
            while ($row = $result->fetch_assoc()) {
                echo '<tr><td>' . $row["codigo"] . '</td><td>' . $row["institucion"] . '</td><td>' . $row["total_activos_robados"] . '</td></tr>';
            }
            echo '</tbody></table>';
        } else {
