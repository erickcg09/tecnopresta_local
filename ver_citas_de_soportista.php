...
if ($tienellave == false){
    ...
    exit(); // Agregar exit para detener la ejecución
}
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Estilos para destinatarios */
        .destinatarios-section {
            margin-top: 30px;
            padding-top: 25px;
            border-top: 2px dashed rgba(123, 131, 235, 0.2);
        }
        ...
        .destinatario-item:hover {
            background-color: rgba(123, 131, 235, 0.08);
            border-color: var(--teams-purple);
        }
        ...
        .add-destinatario-btn:hover {
            background: linear-gradient(135deg, var(--secondary-dark), #17916e);
            transform: translateY(-3px);
        }

        .destinatario-counter {
            display: inline-flex;
            align-items: center;
            justify-content: center;
            width: 28px;
...
<body>
                ...
                <ul class="navbar-nav me-auto">
                    ...
                    <li class="nav-item">
                        <a class="nav-link" href="administrar_citas_teams.php">
                            <i class="bi bi-calendar2-check"></i> Mis citas
                        </a>
                    </li>
                </ul>
            </div>
    ...
    <div class="container">
            ...
            <ol class="instructions-list">
                ...
                <li>
                    ...
                    <div>
                        ...
                        Completa los detalles del evento y añade destinatarios para notificaciones por correo.
                    </div>
                </li>
                <li>
                ...
                <li>
                    ...
                    <div>
                        ...
                        El sistema enviará automáticamente los datos para procesamiento y notificaciones.
                    </div>
                </li>
            </ol>
        </div>
        ...
        <div class="card form-section">
            <form id="eventForm" method="POST" action="procesar_citas_plataforma.php">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="eventSubject" name="eventSubject" class="form-input" 
                ...
                <div class="form-group">
                    ...
                    <input type="datetime-local" id="eventStartTime" name="eventStartTime" class="form-input" required>
                </div>
                ...
                <div class="form-group">
                    ...
                    <input type="number" id="eventDuration" name="eventDuration" class="form-input" min="5" max="1440" value="60" required>
                ...
                <div class="form-group">
                    ...
                    <textarea id="eventDescription" name="eventDescription" class="form-input" 
                ...
                <div class="destinatarios-section">
                    <div class="destinatarios-header">
                        <h3 class="card-title" style="font-size: 1.3rem;">
                            <i class="fas fa-users"></i> Destinatarios de Notificación
                        </h3>
                        <span id="destinatariosCount" style="color: var(--teams-purple); font-weight: 600;">1 destinatario</span>
                    </div>
                    
                    <p style="color: var(--gray); margin-bottom: 20px;">
                        Añade los funcionarios que recibirán notificación por correo con los detalles del evento.
                    </p>
                    
                    <div id="destinatariosContainer">
                        <!-- Primer destinatario -->
                        <div class="destinatario-item" id="destinatario-0">
                            <div class="destinatario-counter">1</div>
                            <div class="destinatario-fields">
                                <div>
                                    <label for="destinatarioNombre0" style="display: block; margin-bottom: 8px; font-weight: 500;">Nombre</label>
                                    <input type="text" id="destinatarioNombre0" name="destinatarios[0][nombre]" class="form-input" placeholder="Nombre del funcionario" required>
                                </div>
                                <div>
                                    <label for="destinatarioCorreo0" style="display: block; margin-bottom: 8px; font-weight: 500;">Correo</label>
                                    <input type="email" id="destinatarioCorreo0" name="destinatarios[0][correo]" class="form-input" placeholder="correo@ejemplo.com" required>
                                </div>
                            </div>
                            <div class="destinatario-actions">
                                <button type="button" class="remove-destinatario" onclick="removeDestinatario(0)" disabled>
                    ...
                    <button type="button" class="add-destinatario-btn" onclick="addDestinatario()">
                        <i class="fas fa-plus-circle"></i> Añadir otro destinatario
                    </button>
                </div>

                <!-- Campos ocultos para datos del evento creado -->
                <div class="hidden-fields">
                    <input type="hidden" id="teamsMeetingLink" name="teamsMeetingLink" value="">
                ...
                <button type="button" id="createEventBtn" class="btn btn-teams" disabled>
        ...
        <div id="resultCard" class="card result-card" style="display: none;">
            ...
            <div class="result-content">
                ...
                <div class="result-item">
                    <div class="result-label">
                        <i class="fas fa-paper-plane"></i> Procesamiento:
                    </div>
                    <div class="result-value">
                        <div id="destinatariosList"></div>
                        <p style="margin-top: 10px; color: var(--success);">
                            <i class="fas fa-check-circle"></i> Los datos del evento han sido enviados al servidor para procesamiento.
                        </p>
                        <p style="margin-top: 5px; color: var(--gray);">
                            Los destinatarios recibirán notificación por correo electrónico con los detalles del evento.
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div class="result-item">
                ...
                <div class="result-value">
                    ...
                    Los datos han sido enviados al sistema para el procesamiento de notificaciones a los destinatarios.
                </div>
            </div>
        </div>
...
<script>
    ...
    function addDestinatario() {
        ...
        const newIndex = destinatariosCount;
        
        const destinatarioHTML = `
            <div class="destinatario-item" id="destinatario-${newIndex}">
                <div class="destinatario-counter">${newIndex + 1}</div>
                <div class="destinatario-fields">
                    <div>
                        <label for="destinatarioNombre${newIndex}" style="display: block; margin-bottom: 8px; font-weight: 500;">Nombre</label>
                        <input type="text" id="destinatarioNombre${newIndex}" name="destinatarios[${newIndex}][nombre]" class="form-input" placeholder="Nombre del funcionario" required>
                    </div>
                    <div>
                        <label for="destinatarioCorreo${newIndex}" style="display: block; margin-bottom: 8px; font-weight: 500;">Correo</label>
                        <input type="email" id="destinatarioCorreo${newIndex}" name="destinatarios[${newIndex}][correo]" class="form-input" placeholder="correo@ejemplo.com" required>
                    </div>
                </div>
                <div class="destinatario-actions">
                    <button type="button" class="remove-destinatario" onclick="removeDestinatario(${newIndex})">
                        <i class="fas fa-times"></i>
                    </button>
                </div>
            </div>
        ...
        container.insertAdjacentHTML('beforeend', destinatarioHTML);
        destinatariosCount++;
        updateDestinatariosCount();
        
        // Habilitar botón de eliminar del primer destinatario si hay más de uno
        if (destinatariosCount > 1) {
            document.querySelector('#destinatario-0 .remove-destinatario').disabled = false;
        }
        
        // Animar el nuevo elemento
        const newElement = document.getElementById(`destinatario-${newIndex}`);
    ...
    function removeDestinatario(index) {
        const element = document.getElementById(`destinatario-${index}`);
        ...
        setTimeout(() => {
            element.remove();
            reindexDestinatarios();
            destinatariosCount--;
            updateDestinatariosCount();
            
            // Deshabilitar botón de eliminar del primer destinatario si solo hay uno
            if (destinatariosCount <= 1) {
                const firstRemoveBtn = document.querySelector('#destinatario-0 .remove-destinatario');
                if (firstRemoveBtn) {
    ...
    // Función para reindexar destinatarios después de eliminar uno
    function reindexDestinatarios() {
        const items = document.querySelectorAll('.destinatario-item');
        items.forEach((item, index) => {
            // Actualizar ID
            item.id = `destinatario-${index}`;
            
            // Actualizar contador
            const counter = item.querySelector('.destinatario-counter');
            counter.textContent = index + 1;
            
            // Actualizar nombres de inputs
            const nombreInput = item.querySelector('input[type="text"]');
            const correoInput = item.querySelector('input[type="email"]');
            const removeButton = item.querySelector('.remove-destinatario');
            
            if (nombreInput) {
                nombreInput.name = `destinatarios[${index}][nombre]`;
                nombreInput.id = `destinatarioNombre${index}`;
            }
            
            if (correoInput) {
                correoInput.name = `destinatarios[${index}][correo]`;
                correoInput.id = `destinatarioCorreo${index}`;
            }
            
            if (removeButton) {
                removeButton.onclick = () => removeDestinatario(index);
            }
        });
    }

    // Actualizar contador de destinatarios
    function updateDestinatariosCount() {
        const countElement = document.getElementById('destinatariosCount');
        countElement.textContent = `${destinatariosCount} ${destinatariosCount === 1 ? 'destinatario' : 'destinatarios'}`;
    }
    ...
    async function acquireTokenSilent() {
        try {
            ...
            if (response && response.accessToken) {
                ...
                // Llenar campos ocultos con información del usuario
                document.getElementById('userEmail').value = currentUser.username;
                document.getElementById('userName').value = currentUser.name || currentUser.username.split('@')[0];
                document.getElementById('accessToken').value = currentToken;
            }
    ...
    async function acquireTokenPopup() {
        try {
            ...
            if (response && response.accessToken) {
                ...
                // Llenar campos ocultos con información del usuario
                document.getElementById('userEmail').value = response.account.username;
                document.getElementById('userName').value = response.account.name || response.account.username.split('@')[0];
                document.getElementById('accessToken').value = currentToken;
            }
    ...
    async function handleFormSubmit() {
        ...
        if (!form.checkValidity()) {
            updateStatus("Por favor, completa todos los campos requeridos correctamente.", "error");
            form.reportValidity();
            return;
        }
        ...
        try {
            ...
            // Obtener destinatarios del formulario para incluirlos como asistentes
            const destinatarios = [];
            document.querySelectorAll('.destinatario-item').forEach(item => {
                const nombre = item.querySelector('input[type="text"]').value;
                const correo = item.querySelector('input[type="email"]').value;
                if (nombre && correo) {
                    destinatarios.push({ 
                        nombre, 
                        correo,
            ...
            // Construir datos del evento con reunión de Teams y ASISTENTES
            const eventData = {
                subject: subject,
                body: {
                    contentType: "HTML",
                ...
                attendees: [
                    // El organizador (requerido para que funcione bien)
                    {
                        emailAddress: {
                    ...
                    // Los destinatarios como asistentes
            ...
            console.log("Enviando solicitud a Microsoft Graph...");
            ...
            console.log("Respuesta recibida de Microsoft Graph:", response.status);
            ...
            if (response.ok) {
                ...
                // Guardar datos del evento para enviar al servidor
                lastMeetingLink = event.onlineMeeting?.joinUrl || "";
                lastEventId = event.id || "";
                
                // Actualizar campos ocultos del formulario
                document.getElementById('teamsMeetingLink').value = lastMeetingLink;
                document.getElementById('eventId').value = lastEventId;
                
                // ✅ Las invitaciones automáticas YA se enviaron porque incluimos
                // a los destinatarios en la creación inicial del evento
                updateStatus("✅ Invitaciones automáticas enviadas a los asistentes", "success");
                
                // Mostrar resultados
                showEventResult(event, subject, startDateTime, duration);
                
                // Enviar formulario al servidor PHP después de 1 segundo
                console.log("Preparando para enviar formulario al servidor...");
                setTimeout(() => {
                    submitFormToServer();
                }, 1000);
            ...
            // Restaurar botón
            const createBtn = document.getElementById("createEventBtn");
            createBtn.innerHTML = '<svg class="teams-icon" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg> Crear Evento con Teams';
            createBtn.disabled = false;
    ...
    /**
     * Función para actualizar el evento con invitados y enviar invitaciones automáticas
     * Se llama después de crear el evento exitosamente (OPCIONAL - backup)
     */
    async function updateEventWithAttendees(eventId, attendeesList) {
        try {
            updateStatus("Enviando invitaciones automáticas a los asistentes...", "warning");
            
            const eventUpdate = {
                attendees: attendeesList.map(attendee => ({
                    emailAddress: {
                        address: attendee.correo,
            ...
            // Actualizar el evento existente con los asistentes
            ...
            if (response.ok) {
                console.log("✅ Evento actualizado con asistentes, invitaciones enviadas");
                updateStatus("✅ Invitaciones automáticas enviadas a los asistentes", "success");
                return true;
            } else {
                const error = await response.text();
                console.warn("⚠️ No se pudieron enviar invitaciones automáticas:", error);
                
                // Manejo específico de error 400 (puede ocurrir si los asistentes ya están incluidos)
                if (response.status === 400) {
                    updateStatus("⚠️ Los asistentes ya estaban incluidos en el evento", "warning");
                } else {
                    updateStatus("⚠️ Evento creado, pero no se enviaron invitaciones automáticas", "warning");
                }
                return false;
            }
        } catch (error) {
            console.error("Error al actualizar evento con asistentes:", error);
            updateStatus("⚠️ Evento creado, pero hubo error al enviar invitaciones automáticas", "warning");
            return false;
        }
    }
    ...
    function showEventResult(event, subject, startDateTime, duration) {
        console.log("Mostrando resultados del evento...");
        ...
        document.querySelectorAll('.destinatario-item').forEach(item => {
            ...
            const correo = item.querySelector('input[type="email"]').value;
            if (nombre && correo) {
                destinatarios.push({ nombre, correo });
            }
        });
        
        let destinatariosHTML = '<ul style="margin-top: 10px; padding-left: 20px;">';
        destinatarios.forEach(dest => {
            destinatariosHTML += `<li style="margin-bottom: 5px;"><strong>${dest.nombre}</strong> (${dest.correo})</li>`;
        });
        destinatariosHTML += '</ul>';
        destinatariosList.innerHTML = destinatariosHTML;
    ...
    function submitFormToServer() {
        console.log("Enviando formulario al servidor PHP...");
        updateStatus("Enviando datos al servidor para procesamiento...", "warning");
        
        // Crear un FormData con los datos del formulario
        const form = document.getElementById('eventForm');
        
        // Enviar el formulario tradicionalmente
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        // Manejar clic en el botón de crear evento
        document.getElementById('createEventBtn').addEventListener('click', function() {
            handleFormSubmit();
        });
        
        // Prevenir envío tradicional del formulario (por si acaso)
        document.getElementById('eventForm').addEventListener('submit', function(e) {
            e.preventDefault();
            console.log("Formulario submit prevenido, usando manejo manual.");
        });
    });
</script>
