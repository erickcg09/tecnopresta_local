...
class sql {
	...
	function conVisitasAsignadas_x_cedula($cedula) {
		
		$pdo = new \PDO(DB_Str, DB_USER, DB_PASS, array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));        
        
		if ($pdo != null){					           

            $consultaSQL = $pdo->prepare('SELECT * FROM visitas_sitio');

            //$consultaSQL->bindParam(':estudiante_id', $estudiante_id, PDO::PARAM_STR);
            //$consultaSQL->bindParam(':fechaYMD', $fechaYMD, PDO::PARAM_STR);
            $consultaSQL->execute();
