...
$miconsulta = "select id_software from t_software where id_tipolicencia='$xeliminar'";
$mirespuesta = $link->query($miconsulta);

if ($mirespuesta->num_rows >= 1) {
		echo '<script language = javascript>
                alert("No se puede eliminar el tipo, es utilizado en software")
                self.location = "formulario_crear_tipo_licencia.php"
                </script>';
} else {
		// sql para eliminar
		$sql = "DELETE FROM t_tipolicencia WHERE id_tipolicencia=$xeliminar";
