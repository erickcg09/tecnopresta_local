...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['activosIzquierda']) && isset($data['activosDerecha'])) {
        ...
        if (isset($data['id_fondos']) && isset($data['codigo']) && isset($data['id_lugar'])) {
            ...
            $subsistema_id = intval($data['subsistema_id'] ?? 0);
            $modulo_id = intval($data['modulo_id'] ?? 0);
            $redirect = 'navegar.php?ruta=formulario_agregar_ubicacion_general_n.php';
            if ($subsistema_id > 0 && $modulo_id > 0) {
                $redirect .= '&subsistema_id=' . $subsistema_id . '&modulo_id=' . $modulo_id;
            }
            $response = array(
