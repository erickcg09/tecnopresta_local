...
<html lang="es">
  <head>
    ...
    <title>Instituciones que accesan</title>
...
<h2 class = "text-center mt-3">Instituciones que accesan TecnoPresta</h2>
...
<?php
// Primera consulta para obtener los códigos únicos de instituciones con al menos un activo
$codigo_sql = "SELECT DISTINCT codigo FROM t_placa WHERE codigo IS NOT NULL";
$codigo_result = $link->query($codigo_sql);

$codigos = [];
if ($codigo_result) {
    while ($row = $codigo_result->fetch_assoc()) {
        $codigos[] = $row['codigo'];
    }
} else {
    echo "Error en la consulta de códigos: " . $link->error;
}

// Segunda consulta para obtener el listado de instituciones basadas en los códigos obtenidos
if (!empty($codigos)) {
    $codigo_list = "'" . implode("','", $codigos) . "'";
    $instituciones_sql = "
        SELECT codigo, institucion 
        FROM t_instituciones 
        WHERE codigo IN ($codigo_list)
        ORDER BY institucion";
    $instituciones_result = $link->query($instituciones_sql);

    $instituciones_count = 0;
    if ($instituciones_result) {
        echo '<table class="table table-striped" id="testTable">';
        echo '<thead><tr><th>Código</th><th>Institución</th></tr></thead>';
        echo '<tbody>';
        while ($row = $instituciones_result->fetch_assoc()) {
            echo '<tr>';
        ...
        echo '</tbody>';
        echo '</table>';
        // Imprimir el total de instituciones en negrita
        echo "<br><strong>Total de instituciones: $instituciones_count</strong>";
    } else {
        echo "Error en la consulta de instituciones: " . $link->error;
    }
} else {
    echo "No se encontraron códigos de instituciones activos.";
}
