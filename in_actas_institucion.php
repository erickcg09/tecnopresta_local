...
if (isset($_GET['codigo']) && isset($_GET['dependencia'])) {
    ...
    echo "<script>
            ...
            window.location.href = 'in_cartepa_actas.php';
          </script>";
    exit();
}
...
$sql = "SELECT t_recepcion_acta_equipo.id_rae, t_recepcion_acta_equipo.etiqueta, t_recepcion_acta_equipo.archivo, t_recepcion_acta_equipo.fecha, t_fondos.fondos 
        FROM t_recepcion_acta_equipo 
        INNER JOIN t_fondos ON t_recepcion_acta_equipo.id_fondos = t_fondos.id_fondos 
        WHERE t_recepcion_acta_equipo.codigo = '$codigo' 
        ORDER BY t_recepcion_acta_equipo.fecha";
...
if ($result->num_rows > 0) {
    ...
    while($row = $result->fetch_assoc()) {
        ...
        echo '<td><a href="in_eliminar_acta.php?id_rae=' . $row['id_rae'] . '&codigo=' . $codigo . '&dependencia=' . $dependencia . '" class="btn btn-danger" onclick="return confirm(\'¿Estás seguro de que deseas eliminar este registro?\')">Eliminar</a></td>';
        echo '</tr>';
    }
    echo '</tbody>';
    echo '</table>';
