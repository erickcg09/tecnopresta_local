...
if (isset($_SESSION['funcionario'])) {
    ...
    $clasedepuesto = mysqli_real_escape_string($link, $resultado['Clase_Puesto']);
    $especialidad = mysqli_real_escape_string($link, $resultado['Especialidad']);

    // Consultar la existencia del usuario en la lista blanca
    $query = "SELECT id_lista_blanca FROM t_lista_blanca WHERE cedula=?";
    ...
    if ($check_user > 0) {
        ...
        $sql = "SELECT id_rol FROM t_lista_blanca WHERE cedula=?";
        ...
        $_SESSION['clase'] = $clasedepuesto;
        $_SESSION['especialidad'] = $especialidad;

        // Redirigir a la página de inspector.php
