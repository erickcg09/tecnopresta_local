...
if (empty($marca)) {
    $_SESSION['error'] = "Debe ingresar una marca";
...
if ($imagen['size'] > $limite_kb * 1024) {
    $_SESSION['error'] = "El tamaño del logo excede el límite de $limite_kb KB";
    header('Location: formulario_crear_marca.php');
    exit();
}

// Verificar si la marca ya existe
$stmt = $link->prepare("SELECT id_marca FROM t_marca WHERE marca = ?");
...
if ($stmt->num_rows > 0) {
    $stmt->close();
    $_SESSION['error'] = "La marca ya existe en la base de datos";
    header('Location: formulario_crear_marca.php');
    exit();
}
$stmt->close();
...
$nombre_unico = uniqid('logo_') . '.' . $permitidos[$imagen['type']];
$ruta = "ico/" . $nombre_unico;

// Mover archivo
if (!move_uploaded_file($imagen['tmp_name'], $ruta)) {
    error_log("Error al mover el archivo subido: " . $imagen['name']);
    $_SESSION['error'] = "Error al guardar el logo";
    header('Location: formulario_crear_marca.php');
    exit();
}
...
$stmt = $link->prepare("INSERT INTO t_marca (marca, logo) VALUES (?, ?)");
$stmt->bind_param("ss", $marca, $nombre_unico);
