...
<h3>Revisi&oacute;n de Activo</h3><a href="ayuda.html#au">[ <span class="icon icon-lifebuoy"></span> Ayuda ]</a><a>&nbsp;&nbsp;</a> <a href="contactenos.php?rep=Error en Agregar Ubicaci&oacute;n del Activo">[ <span class="icon icon-envelop"></span> Reportar Incidencia / Error ]</a>
<br><br>
...
<form action="formulario_reportar_ubicacion.php" method="post" >
...
<table class="table table-hover">
	...
	<th>Serial</th>
	<th>Revisado</th>
	<th>Detalle de la revisi&oacute;n</th>
        <tbody class="BusquedaRapida">
	<?php
$consulta=mysqli_query($link,"SELECT Ta.id_activo, Tg.clase, Tm.marca, Ta.modelo, Tc.color, Tp.id_placa, Tp.placa, Tp.serial, Tp.id_estado, Tp.codigo, Tp.revisado, Tp.id_fondos
		 FROM t_activo Ta
		 ...
		 WHERE Tp.codigo = '".$codigo."' AND Ta.id_activo = '".$activo."' AND Tp.id_fondos = '".$fondos."'
	...
	<tr>
		...
		<?php $revisado=$activos['revisado'];?>
		<td><input type="checkbox" <?php if($revisado=="1"){?> checked="true" <?php } ?> /></td>
                <td><a class="btn btn-dark" href="formulario_revision_detalle.php?gps=<?php echo $activos['id_placa']?>&varcodigo=<?php echo $codigo?>&varactivo=<?php echo $activo?>&varfondos=<?php echo $fondos?>&varquest=<?php echo $quest?>" role="button"><span class="icon icon-pencil2"></span></a></td>
	</tr>
	<?php }
	mysqli_close($link);	
	?>
