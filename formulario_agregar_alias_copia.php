...
<h3>Agregar Alias y Numeraci&oacute;n a los Activos</h3><a href="ayuda.html#aan">[ <span class="icon icon-lifebuoy"></span> Ayuda ]</a><a>&nbsp;&nbsp;</a> <a href="contactenos.php?rep=Error en Agregar Alias y Numeraci&oacute;n">[ <span class="icon icon-envelop"></span> Reportar Incidencia / Error ]</a>
<br><br>
...
<form action="actualizar_alias_cero.php" method="post" >

<table class="table table-hover">

	<h2></h2>
	<th><div class="custom-control custom-checkbox"><input type="checkbox" id="selectall" class="custom-control-input"><label class="custom-control-label" for="selectall">Todos</label></div></th>
	<th>Activo</th>
	<th>Marca</th>
	<th>Modelo</th>
    <th>Color</th>
 ...
 <input type="hidden" id="id_alias" name="id_alias">
  ...
  <div class="form-group">
	<select id="myDropdown" name="myDropdown">
                          <option value="0">Seleccione Alias:</option>   
    			<?php  

     			 $regres=mysqli_query($link,"select * from t_alias WHERE codigo = '".$logcodigo."' order by alias") or
        		...
        		<option value="<?php echo $regr['alias_id']; ?>" data-imagesrc="img/alias/<?php echo $regr['alias_imagen']; ?>"
	...
	<th colspan="2"><button class="btn btn-dark btn-lg btn-block" type="submit" name="btnActualizar"> <span class="icon icon-floppy-disk"></span> </button></th>
...
$consulta=mysqli_query($link,"SELECT Ta.id_activo, Tg.clase, Tm.marca, Ta.modelo, Tc.color, Tp.id_placa, Tp.placa, Tp.serial, Tp.codigo, Tp.activo, Tp.numero_activo, Tp.alias_id
		 ...
		 WHERE Tp.codigo = '".$logcodigo."' AND Tp.activo = '".$activado."' AND Tp.alias_id = '".$cero."'
	...
	<tr>
		...
		<td>Número de Activo <input type="text" name="numeracion<?php echo $activos['id_placa']; ?>" size=5 value="<?php echo $activos['numero_activo'];?>"></td>
...
$('#myDropdown').ddslick({
    onSelected: function(data){
       document.getElementById("id_alias").value = data.selectedData.value; 
    }   
});
</script>
</body>
